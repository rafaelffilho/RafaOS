...
static void expand(uint32_t new_size, heap_t *heap) {
	...
	while (i < new_size) {
		alloc_frame(get_page(heap->start_address + i, 1, kernel_directory),
		            heap->supervisor, heap->read_only);
		i += 0x1000;
	}
...
heap_t *create_heap(uint32_t start, uint32_t end, uint32_t max,
                    uint8_t supervisor, uint8_t read_only) {
...
void *alloc(uint32_t size, uint8_t page_align, heap_t *heap) {
	...
	if (orig_hole_size < sizeof(header_t) + sizeof(footer_t)) {
	...
	if (orig_hole_size - new_size > 0) {
		header_t *hole_header =
		  (header_t *) (orig_hole_pos + sizeof(header_t) + size + sizeof(footer_t));
		hole_header->magic   = HEAP_MAGIC;
		hole_header->is_hole = 1;
